...
## Persona Layer — When to Skip

The `adapt_persona` node rewrites prose answers in role-appropriate language. It must NOT run on structured output.

**Rule: agents that return structured cards set `skip_persona=True` in `structured` and handle role adaptation themselves.**

| Agent output type | `skip_persona` | Who handles role adaptation |
|---|---|---|
| Prose answer (CrossSourceAgent) | `False` — persona runs | `adapt_persona` node |
| Structured table (RiskAgent, PRReviewAgent) | `True` — persona skipped | Agent's own `_role_summary()` |
| HITL proposal card (any agent) | Implicit skip via `hitl_required=True` | No adaptation — card format is fixed |
| Error/validation message (ticket not found, etc.) | `True` via `_SKIP_PERSONA_SIGNALS` match | Returned verbatim |

Why: the persona layer is a second LLM call designed for free text. When it receives a markdown table it converts it to prose bullets, destroying the information structure. Structured cards always bypass it and write their own role-specific intro line.

`skip_persona` must be declared in `SDLCState` — LangGraph silently drops any key not in the TypedDict.
