...
def as_list(result: object) -> list:
    """
    Normalize a call_mcp_tool() result for a tool whose return type is a list.

    langchain_mcp_adapters collapses a single MCP content block down to its bare
    value (see its _convert_call_tool_result: `len(text_contents) == 1` -> unwrap
    the list). Our MCP server emits one content block per list item, so a list-
    returning tool with EXACTLY ONE result comes back as a bare dict, not a
    one-item list — every list-returning tool is vulnerable to this whenever it
    has exactly one result. Two call sites (risk_agent, admin/router) had already
    independently rediscovered and locally patched this; this is the one place
    every caller should use instead, so it isn't rediscovered a third time.
    """
    if isinstance(result, list):
        return result
