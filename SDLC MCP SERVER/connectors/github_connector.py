...
class GitHubConnector(BaseMCPConnector):
    ...
    async def _get_pr_ci_status(self, pr_number: int) -> str:
        try:
    ...
    async def list_open_prs(self, repo: str = "") -> list[dict]:
        ...
        try:
            ...
            results = []
            for pr in prs:
                pr["_ci_status"] = await self._get_pr_ci_status(pr["number"])
                try:
                    r_files = await self.http.get(
    ...
    async def get_pr_details(self, pr_id: str) -> dict | None:
        ...
        try:
            r = await self.http.get(f"{base}/pulls/{number}")
            ...
            files_r, reviews_r, review_cmts_r, issue_cmts_r, ci = await asyncio.gather(
                self.http.get(f"{base}/pulls/{number}/files"),
                self.http.get(f"{base}/pulls/{number}/reviews"),
                self.http.get(f"{base}/pulls/{number}/comments"),
                self.http.get(f"{base}/issues/{number}/comments"),
                self._get_pr_ci_status(int(number)),
    ...
    async def is_collaborator(self, username: str) -> bool:
        ...
        """
        try:
            r = await self.http.get(f"{_API_BASE}/repos/{self._owner}/{self._repo}/collaborators/{username}")
    ...
    async def assign_reviewer(self, pr_id: str, reviewer: str) -> dict:
        ...
        try:
            ...
            if reviewer.lower() not in actual:
                ...
                msg = (
                    f"GitHub accepted the request but did not add '{reviewer}' as a reviewer "
            ...
            return {"success": True, "pr": pr_id, "reviewer": reviewer, "status": "assigned"}
            ...
            return {"success": False, "error": message or f"HTTP {code}", "pr": pr_id, "reviewer": reviewer, "status": "error", "http_code": code, "message": message}
        except Exception:
            logger.exception("GitHubConnector.assign_reviewer failed for %s", pr_id)
            return {"success": False, "error": "request failed", "pr": pr_id, "reviewer": reviewer, "status": "error", "http_code": 0, "message": ""}
    ...
    async def approve_pr(self, pr_id: str, approver: str = "") -> dict:
        ...
        try:
            ...
            # NOTE: the review is attributed to the token owner by GitHub;
            # `approved_by` is audit metadata echoed back, never sent upstream.
            return {"success": True, "pr": pr_id, "status": "APPROVED", "approved_by": approver}
        except httpx.HTTPStatusError as exc:
            code, message = exc.response.status_code, _error_message(exc)
            logger.exception("GitHubConnector.approve_pr failed for %s (HTTP %s): %s", pr_id, code, message)
            return {"success": False, "error": message or f"HTTP {code}", "pr": pr_id, "status": "error", "http_code": code, "message": message}
        except Exception:
            logger.exception("GitHubConnector.approve_pr failed for %s", pr_id)
            return {"success": False, "error": "request failed", "pr": pr_id, "status": "error", "http_code": 0, "message": ""}
    ...
    async def request_changes_pr(self, pr_id: str, body: str = "", reviewer: str = "") -> dict:
        ...
        try:
            ...
            return {"success": True, "pr": pr_id, "status": "CHANGES_REQUESTED", "reviewer": reviewer, "body": body}
        except httpx.HTTPStatusError as exc:
            code, message = exc.response.status_code, _error_message(exc)
