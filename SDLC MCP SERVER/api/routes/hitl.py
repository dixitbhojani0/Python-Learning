...
async def approve_hitl(
    ...
    if action_type == "create_ticket":
        ...
        if approved:
            ...
            # Surface GitHub's own error rather than guessing the reason — a 403 here
            # can mean an under-scoped token just as easily as a self-approval
            # restriction, and a wrong guess is worse than none (see assign_reviewer).
            result_text = (
                f"❌ **Could not approve {pr_number}.**\n\n"
                f"GitHub said: _{result['message']}_ (HTTP {result.get('http_code', '?')})\n\n"
                f"See GitHub's message above for the exact reason, or approve manually in GitHub."
            )
        else:
        ...
        pr_number = proposal.get("pr_number", "N/A")
        pr_title  = proposal.get("pr_title", "")
        body      = (proposal.get("review_data") or {}).get("concerns", "").strip() \
                    or "Automated review found issues that need addressing before merge."
        result    = {}
        try:
            raw       = await call_mcp_tool("github_request_changes_pr", {"pr_id": pr_number, "body": body, "reviewer": user.name})
            result    = raw if isinstance(raw, dict) else {}
            requested = str(result.get("status", "")).upper() == "CHANGES_REQUESTED"
            logger.info("hitl/approve: PR %s request-changes via MCP → %s", pr_number, result.get("status"))
        except Exception:
            requested = False
            logger.exception("hitl/approve: MCP github_request_changes_pr failed for PR %s", pr_number)
        if requested:
            result_text = (
                f"❌ **Changes requested on {pr_number} by {user.name}.**\n\n"
                f"PR: _{pr_title}_\n\n"
                f"> {body}\n\n"
                f"The author has been notified via GitHub to address these concerns before merge."
            )
        elif result.get("message"):
            result_text = (
                f"❌ **Could not request changes on {pr_number}.**\n\n"
                f"GitHub said: _{result['message']}_ (HTTP {result.get('http_code', '?')})\n\n"
                f"See GitHub's message above for the exact reason, or submit manually in GitHub."
            )
        else:
            result_text = f"⚠️ **Could not submit the changes-requested review on {pr_number}** — GitHub connector unavailable."
...
async def _execute_edit_ticket(proposal: dict, approver_name: str) -> str:
    """Update a Jira ticket's title/description over MCP (`jira_update_ticket`)."""
    ticket_id = proposal.get("ticket_id", "UNKNOWN")
    field     = proposal.get("field", "description")
    new_value = proposal.get("new_value", "")

    args = {"ticket_id": ticket_id, "summary": "", "description": ""}
    args["summary" if field == "title" else "description"] = new_value

    try:
        result = await _mcp_write("jira_update_ticket", args)
        if result.get("success"):
            logger.info("hitl/approve: %s field '%s' updated via MCP", ticket_id, field)
            # No truncation here — this confirms what was ACTUALLY saved to Jira, so
            # cutting it off (previously [:300]) made a complete update look partial/
            # broken for any description over 300 chars, even though the real ticket
            # was correct. The proposal CARD still previews truncated (pre-approval,
            # compactness is fine there); this is the post-approval receipt.
            return (
                f"✅ **{ticket_id} updated.**\n\n"
                f"**{field.capitalize()}** changed to:\n\n"
                f"> {new_value}\n\n"
                f"Approved by: {approver_name}."
            )
        logger.warning("hitl/approve edit_ticket via MCP: %s", result.get("error"))
    except Exception:
        logger.exception("hitl/approve: _execute_edit_ticket via MCP raised")
...
async def _execute_assign_reviewer(proposal: dict) -> str:
    ...
    Extracted from an inline approve_hitl() branch that discarded the MCP call's
    result entirely and unconditionally reported "✅ assigned" — including when
    GitHub rejected the request (e.g. reviewer isn't a repo collaborator). Standalone
    so it can be unit tested like the other _execute_* handlers; that inline form
    couldn't be tested at all, which is exactly how the bug went unnoticed.
    """
    pr_number = proposal.get("pr_number", "N/A")
    ...
    result = {}
    try:
        result = await _mcp_write("github_assign_reviewer", {"pr_id": pr_number, "reviewer": reviewer})
        logger.info("hitl/approve: reviewer '%s' assign to %s via MCP → %s", reviewer, pr_number, result.get("status"))
    except Exception:
        logger.exception("hitl/approve: MCP github_assign_reviewer raised for %s", pr_number)
    ...
    if result.get("message"):
        # Surface GitHub's own error rather than guessing the reason from the status
        # code alone — e.g. a 403 "Resource not accessible by personal access token"
        # means the token needs more permissions, NOT that the reviewer isn't a
        # collaborator. A wrong guess here is worse than no guess at all.
        return (
            f"❌ **Could not assign `{reviewer}` to {pr_number}.**\n\n"
            f"GitHub said: _{result['message']}_ (HTTP {result.get('http_code', '?')})\n\n"
            f"See GitHub's message above for the exact reason, or assign manually in GitHub."
        )
    return (
        f"⚠️ **Could not assign `{reviewer}` to {pr_number}** — GitHub connector unavailable. "
        f"Please try again or assign manually in GitHub."
    )
