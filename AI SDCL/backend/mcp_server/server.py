...
from backend.mcp_server.tools import (
    ...
    github_tools,
    slack_tools,
    # confluence_tools omitted — Confluence content is already indexed in Qdrant.
    # RAG retrieval is faster and offline-safe. Re-enable only if you need
    # live page lookup by ID that isn't yet ingested.
    # teams_tools, drive_tools — DISABLED (connectors disabled in mcp_registry.yaml).
    # Re-add here AND flip enabled: true in the yaml to bring them back.
)

_registry = MCPRegistry()
# Read tools — safe for the autonomous gather loop.
# Confluence intentionally excluded — served from Qdrant vector index instead.
...
from backend.mcp.constants import is_write_tool   # shared — no longer imports from mcp_client
