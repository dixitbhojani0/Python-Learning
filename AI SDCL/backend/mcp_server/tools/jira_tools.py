...
def register(mcp: Any, registry: Any) -> None:
    ...
    async def jira_find_similar_tickets(title: str, project: str = "") -> list[dict]:
        ...
        Call this BEFORE jira_create_ticket so the host can surface possible
        duplicates to the user and confirm before creating. The MCP host handles
        the confirm step; this tool is read-only and side-effect free.

        Args:
            title: candidate ticket title to compare against open tickets.
            project: Jira project key (e.g. "SDLC"); empty = default project.

        Returns: list of matching tickets (same shape as jira_search_tickets).
        """
...
def register_writes(mcp: Any, registry: Any) -> None:
    ...
    async def jira_create_ticket(
        ...
        Tip: pre-check duplicates with `jira_find_similar_tickets(title)` first.
        ...
        Args:
            ...
            sprint_id: one of:
                - numeric id (e.g. "123") → use as-is;
                - "current" / "active" / "" / a sprint NAME → auto-resolve to the
                  project's active sprint id;
                - "backlog" / "none" / "no" → explicitly land in the backlog.
            project: Jira project key for sprint auto-resolution; empty = default.

        Returns: {"id": "<KEY>", "url": ..., "sprint_resolved": bool, "note": "..."}.
        sprint_resolved=true means the ticket is on the active sprint board.
        """
        ...
        if sp.isdigit():
            ...
            pass   # explicit Backlog — no auto-resolve, no note
        else:
            # empty / "current" / "active" / a sprint NAME → resolve active sprint id
            ...
            if sid:
                ...
                logger.warning("jira_create_ticket: sprint_id=%r unresolved — falling back to backlog", sp)
        ...
        # Belt-and-braces: if we have a sprint id and the ticket was created,
        # also POST it to /rest/agile/1.0/sprint/{id}/issue. Jira Cloud sometimes
        # drops the customfield_10020 set at create time (custom-field id varies
        # per board), and this endpoint reliably adds the issue to the sprint.
