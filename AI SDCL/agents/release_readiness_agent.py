...
HITL is required only for a GO verdict — releasing is the high-stakes
irreversible action that needs human sign-off. A NO_GO has nothing to
approve (hitl.py hard-blocks approving a NO_GO release regardless), so
it's returned as an informational assessment with no Approve/Reject buttons.
...
def _format_release_response(release_data: dict) -> str:
    ...
    if verdict == "GO":
        closing = "_Click **Approve** to confirm release readiness or **Reject** to cancel._"
    else:
        # NO_GO is not a decision point — hitl.py hard-blocks approving a NO_GO release,
        # so Approve/Reject buttons here would just error out or state the obvious.
        # Informational only; re-running the check after fixing blockers is the real next step.
        closing = "_Resolve the blockers above, then re-run the release readiness check._"
...
class ReleaseReadinessAgent(BaseAgent):
    """
    Release readiness assessment — HITL proposal only for a GO verdict.
    ...
    Flow:
      ...
      5. Parse JSON → format response (proposal card for GO, plain assessment for NO_GO)
      6. Return AgentPayload(hitl_required=<verdict == "GO">)
    """
    ...
    async def _fetch_mcp_data(self, project: str) -> tuple[dict, list[dict], list[dict]]:
        ...
        try:
            ...
            jira_tickets = as_list(results[1])
            github_prs   = as_list(results[2])
    ...
    async def run(self, state: SDLCState) -> AgentPayload:
        ...
        # HITL is only a real decision point for a GO verdict (should we actually release?).
        # A NO_GO has nothing to approve — hitl.py's release_approval handler hard-blocks
        # approving a NO_GO anyway, so offering the buttons here is confusing dead-end UI.
        is_go = release_data.get("verdict") == "GO"
        ...
        return AgentPayload(
            ...
            sources=all_sources,
            hitl_required=is_go,
            hitl_proposal=proposal if is_go else {},
        )
