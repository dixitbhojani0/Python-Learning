...
from backend.orchestrator.evaluation import evaluate_response
...
async def adapt_persona(state: SDLCState) -> dict:
    ...
    # skip_persona is a structured flag set by the producing agent (system errors,
    # validation messages, review cards) — no response-text sniffing here.
    if state.get("hitl_required") or state.get("skip_persona") or not response.strip() or is_no_data:
        logger.debug(
            "adapt_persona: skipping — hitl_required=%s, skip_persona=%s, empty=%s, no_data=%s",
            state.get("hitl_required"), state.get("skip_persona"), not response.strip(), is_no_data,
        )
        return {}
    ...
    # Scoring + reflection retry live in orchestrator/evaluation.py; the chat route
    # reads the scores off the returned state.
    adapted, faith, relev = await evaluate_response(state, adapted, persona, get_provider())
